...
export default class CellsContextMenu{
    ...
    async #buildExtraMenu(event, processing = true, lab = false) {
        ...
        this.createProcessingRejectOrders = async (event, rejected = false, extraElement) => {
            this.extraMenuCloser(event, true);
            if (!this.chosenBatch) {
                const res = await this.createProRejOrder(this.targetCell.elementData, extraElement, rejected);
            } else {
