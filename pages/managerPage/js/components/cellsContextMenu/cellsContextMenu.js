...
import { BACK_URLS, FLASH_MESSAGES, LABORATORY_NAME, ORDER_MOVE_TO_PROCESSING, ORDER_MOVE_TO_REJECTED, ORDER_TYPES_TRANSLATE, TESTS_FAILED, TESTS_NOT_DONE } from "../../constants.js";
...
export default class CellsContextMenu{
    ...
    async createProRejBulkOrders(placementData, extraElement, batchNumber, processing) {
        let createOrderURL = `${BACK_URLS.CREATE_MOVE_TO_BULK_PRO_REJ_ORDERS}`;
        let orderName = 'AutoGenerated-BulkMoveToProcessing';
        let orderDesc = `AutoGenerated-BulkMoveToProcessing`;
        const orderReqBody = {
            'orderName': orderName,
            'orderDescription': orderDesc,
            'batchNumber': batchNumber,
            'placement_id': placementData['placementId'],
            'placementType': placementData['type'],
            'destination': {
                'placementType': 'grid',
        ...
        if (processing) {
            orderReqBody['orderType'] = ORDER_MOVE_TO_PROCESSING;
        } else {
            orderReqBody['orderType'] = ORDER_MOVE_TO_REJECTED;
        }
        const resp = this.#postOrder(createOrderURL, orderReqBody);
        return resp
    }
    ...
    async #buildExtraMenu(event, processing = true, lab = false) {
        ...
        this.createProcessingRejectOrders = async (event, processing = false, extraElement) => {
            ...
            if (!this.chosenBatch) {
                ...
                const res = await this.createProRejBulkOrders(
                    this.targetCell.elementData['placement'], extraElement, this.chosenBatch, processing
                );
            }
        }
