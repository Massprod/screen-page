...
export default class CellsContextMenu{
    ...
    async createProBulkOrders(elementData, extraElement, batchNumber) {
        let createOrderURL = `${BACK_URLS.CREATE_MOVE_TO_PROCESSING_BULK_ORDERS}`;
        ...
        const orderReqBody = {
            ...
            'placement_id': elementData['placement']['placementId'],
            'destination': {
                'placementType': 'grid',
                'placementId': elementData['placement']['placementId'],
                'elementName': extraElement,
    ...
    async #buildExtraMenu(event, processing = true, lab = false) {
        ...
        this.createProcessingRejectOrders = async (event, processing = false, extraElement) => {
            ...
            if (!this.chosenBatch) {
                ...
                if (processing) {
                    const res = await this.createProBulkOrders(this.targetCell.elementData, extraElement, this.chosenBatch);
                }
            }
        }
